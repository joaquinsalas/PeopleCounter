...
//captura imágenes de profundidad, detecta personas y envía a la
//tarjeta maestra las coordenas y valores de profundidad de las
//detecciones, si capturó una imagen y el tiempo de captura en milisegundos
//para compilar:
// g++ slave.cpp -o slave `pkg-config --cflags --libs opencv` -I/home/pablo/OpenNI-Linux-x64-2.2/Include
//-L/home/pablo/OpenNI-Linux-x64-2.2/Redist -lOpenNI2 -O3
...
int main(int argc, char **argv) {
  ...
  while( true ){
    ...
    write(sockfd, "1", 1);  //avisa que recibió la orden
    ...
    //envía los datos de detecciones
...
void send_data(int* u, int* v, int* z, int sockfd, int rcap){
  //envía las coordenadas de las detecciones
  int buffer[90];
  char res[2];
...
void detection(int* ud, int* vd, int* zd, Mat Z, Mat fgmask){
  //realiza la detección y anota las coordenadas
  //parámetros de detección
  //área de la cabeza en cm^2
  ...
  //área mínima de la cabeza de una persona en pixeles en la imagen
  float fx = 295;
  float amin = 300.0;
  ...
  //detección de blobs y sus contornos
  ...
  for(int i=0; i<(int)contours.size(); i++){
	...
	Area =  3.1416*(rc*rc)*(zc*zc)/(fx*fx);  //área del cŕculo en cm^2;
