...
//para compilar: g++ master.cpp json.cpp -o master -O3

int main(int argc, char **argv) {
  //parámetros de las cámaras
  //R, T = Rotación y translación respecto a la cámara de referencia
  //c0, c1 = constantes para convertir disparidad a profundidad
  int nc = 3;  //número de cámaras
  cameraParameters cam[nc];
  //número de observaciones por imagen en todas las cámaras
  int nd = 30*nc;
  read_parameters(cam, nc);
  int u[nd], v[nd], z[nd];
  tracklet tr[nd];

  //crea el archivo de eventos si no existe 
  int fd = 0;
  char fn_eventos[] = "/var/www/html/CounterPeopleWebAppInterface/contador/eventos.txt";
  FILE* fid = fopen(fn_eventos,"r");
  if( fid==0 ){ fd = 1; }
  fclose(fid);
  if( fd ){
    fid = fopen(fn_eventos,"w");
    fprintf(fid,"{\"entradas\":\"%d\",\"salidas\":\"%d\"}",0,0);
    fclose(fid);
  }

  //archivo para anotar la velocidad de procesamiento de cuadros
  char fn_fps[] = "/var/www/html/CounterPeopleWebAppInterface/contador/fps.txt";
  fid = fopen(fn_fps,"w");
  fprintf(fid,"{\"fps\":\"%0.1f\"}",0.0);
  fclose(fid);

  //tiempo inicial con resolución de milisegundos
  timeb tmb;
  ftime( &tmb );
  ...
  int stcams = 0, stevents = 0, kfps = 0;
  ...
  while( true ){
    ...
    //finaliza el programa si se inicio el programa terminar
    int proc_id = system("pidof terminar");
    if( proc_id != 256 ){
      //envia orden para terminar el programa en las tarjetas esclavas
      for(int ncam=0; ncam<nc; ncam++)
        write(sockfd[ncam], "2", 1);
      system("pkill terminar");
      break;
    }
    
    //obtiene las detecciones actuales de cada cámara y revisa el estado de las cámaras
    get_data(u, v, z, nc, sockfd, estado);
    if( (stcams==0) && (nw->tm_sec<10) ){
      int stcam[nc];
      for(int i=0; i<nc; i++){
        stcam[i] = 1;
        if( estado[2*i]==0 || estado[2*i+1]==0 )
          stcam[i] = 0;
      }
      fid = fopen("status.json", "w");
      fprintf(fid, "{\n");
    ...
    int E = 0, S = 0, maxcount = 1000000, Et, St;
    if( update_count(E, S, tr, nd) ){
      //actualiza los archivos JSON para la página Web
      create_directories(nw);
      update_json_general(E, S, nw);
      update_json_year(E, S, nw);
      update_json_month(E, S, nw);
      update_json_day(E, S, nw);

      //anota en el archivo la salida del contador
      fid = fopen(fn_eventos,"r");
      fscanf(fid,"{\"entradas\":\"%d\",\"salidas\":\"%d\"}",&Et,&St);
      fclose(fid);
      Et += E;  St += S;
      if( (Et > maxcount) || (St > maxcount) ){ Et = 0;  St = 0; }
      fid = fopen(fn_eventos,"w");
      fprintf(fid,"{\"entradas\":\"%d\",\"salidas\":\"%d\"}\n",Et,St);
      fclose(fid);
    }

    //anota la velocidad de procesamiento de cuadros
    kfps++;
    if( kfps>10 ){
      if( ta<t ){ fps = 1.0/(t - ta); }
      else{ fps = 0.0; }
      fid = fopen(fn_fps, "w");
      fprintf(fid,"{\"fps\":\"%0.1f\"}",fps);
      fclose(fid);
      kfps = 0;
    }
    ta = t;

    //reestablece el archivo de eventos en cero al inicio del dia
    if( (stevents==0) && (nw->tm_hour<12) ){
      fid = fopen(fn_eventos,"w");
      fprintf(fid, "{\"entradas\":\"%d\",\"salidas\":\"%d\"}\n", 0, 0);
      fclose(fid);
      stevents = 1;
    }
    if( nw->tm_hour>12 ){ stevents = 0; }
  }
  for(int i=0; i<nc; i++)
    close( sockfd[i] );
  return 0;
